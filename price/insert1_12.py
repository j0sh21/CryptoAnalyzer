...
def create(symbol_i):
    ...
    cur.execute(e)
    e = "create or replace view MAX_H_" + str(symbol_i) + " as select DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H') as Tag_Stunde,  DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%H:%i') as Uhrzeit, max(price) as maxp, fetch_id INT from "+ str(symbol_i)+" where (price, DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H')) in (Select max(price), DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H') from "+ str(symbol_i)+" group by DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H'))group by Tag_Stunde;\n"
    cur.execute(e)
    e = "create or replace view MIN_H_" +str(symbol_i) + " as select DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H') as Tag_Stunde,  DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%H:%i') as Uhrzeit, min(price) as minp, fetch_id INT from " + str(symbol_i)  + " where (price, DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H')) in (Select min(price), DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H') from " + str(symbol_i)  + " group by DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y:%H'))group by Tag_Stunde;\n"
    cur.execute(e)
    e = "create or replace view MAX" + str(symbol_i) + " as select DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y') as Tag,  DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%H:%i') as Uhrzeit, max(price) as maxp,fetch_id INT from "+ str(symbol_i)+" where (price, DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y')) in (Select max(price), DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y') from " + str(symbol_i) + " group by DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y'))group by Tag;\n"
    cur.execute(e)
    e = "create or replace view MIN" + str(symbol_i) + " as select DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y') as Tag, DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%H:%i') as Uhrzeit,  min(price) as minp, fetch_id INT from "+ str(symbol_i)+" where (price, DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y')) in (Select min(price), DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y') from " + str(symbol_i) + " group by DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y'))group by Tag;\n"
    cur.execute(e)
    e = " create or replace view AVG" + str(symbol_i) + " as select DATE_FORMAT(ADDDATE(datum, INTERVAL 2 HOUR), '%d.%m.%Y')as Tag, avg(price) as AVGP, fetch_id INT from " + str(symbol_i) + " group by Tag;\n"
    cur.execute(e)
    con.commit()
